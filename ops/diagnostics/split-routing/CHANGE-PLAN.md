...
Не записываются query string, cookies, authorization headers, User-Agent или полный стандартный access log. Клиентские TSV/ZIP и серверный лог удаляются вручную после принятия результата; рекомендуемый максимальный retention для одноразового теста — 7 дней. Автоматического удаления по истечении срока нет. Владелец отдельно проверяет и удаляет рабочие копии, файлы в Telegram и серверный лог. Сертификат может оставаться неиспользуемым до согласованного certificate cleanup.
